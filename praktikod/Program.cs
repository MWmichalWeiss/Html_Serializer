...
foreach (string s in htmlLines)
{
    ...
    if (firstWord == "html")
    {
        ...
        rootElement.InnerHtml = new StringBuilder();
    ...
    else
    {
        if (HtmlHelper.Instance.AllTags.Contains(firstWord))
        {
            ...
            //Console.WriteLine("the father is: "+newElement.Parent.Name+" and the children is: "+newElement.Name);

            if (firstWord[firstWord.Length - 1] != '/' && !HtmlHelper.Instance.OneTag.Contains(firstWord))
            {
                currentElement = newElement;
                currentElement.Children = new List<HtmlElement>();
                currentElement.InnerHtml = new StringBuilder();
            }
        }
        else
        {
            ...
            {
                if (s[i] == ' ' && s[i + 1] != ' ')
                    currentElement.InnerHtml.Append(s[i]);
            }
            //currentElement.InnerHtml.Append(s);
        }
...
var s2 = new Selector();
var results = rootElement.FindElementsUsingSelector(Selector.ConvertToSelector("ul.nav.navbar-nav"));
